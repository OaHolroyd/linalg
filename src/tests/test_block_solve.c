...
int main(void) {
  ...
  SUBTEST("block solve") {
    ...
    const size_t work_size = n * m + ((n > m) ? n : m);
  ...
  SUBTEST("block solve simplified") {
    const int n = 5;
    const int m = 5;
    const int nm = n + m;
    double **R = malloc_d2d(nm, nm);
    ...
    // overwrite D with the Schur complement
    for (int i = 0; i < m; i++) {
      for (int j = 0; j < m; j++) {
        for (int k = 0; k < n; k++) {
          D[i][j] -= C[i][k] * B[k][j];
    ...
    const size_t work_size = ((n > m) ? n : m);
    double *work = malloc(work_size * sizeof(double));
    int *pivm = piv + n;
    err = block_solve_simplified(B[0], C[0], D[0], a, b, pivm, work, n, m);
    REQUIRE_BARRIER(err == 0);
